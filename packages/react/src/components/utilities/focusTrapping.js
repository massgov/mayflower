...
export default ({
  ...
  const action = {
    tab: keyEvent === 'Tab', // tab
    shift: keyEvent.shift, //shift
    esc: keyEvent === 'Esc' || keyEvent === 'Escape', // esc
    left: keyEvent === 'Left' || keyEvent === 'ArrowLeft', // left arrow
    right: keyEvent === 'Right' || keyEvent === 'ArrowRight', // right arrow
    up: keyEvent === 'Up' || keyEvent === 'ArrowUp', // up arrow
    down: keyEvent === 'Down' || keyEvent === 'ArrowDown' // down arrow
  };
  ...
  if (modal) {
    ...
    const isTabPressed = tab || right;

    if (!isTabPressed) {
      return;
    }

    if (shift || left) { // if shift keyEvent pressed for shift + tab combination
      if (document.activeElement === firstFocusableElement) {
        lastFocusableElement.focus(); // add focus for the last focusable element
        e.preventDefault();
      }
    } else if (document.activeElement === lastFocusableElement) { // if tab keyEvent is pressed
