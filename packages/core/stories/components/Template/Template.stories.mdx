...
> To use a different variant of Header or Footer component, refer to <button onClick={linkTo(generateTitle('Header'))}>Header</button> and <button onClick={linkTo(generateTitle('Footer'))}>Footer</button>, and replace the HTML, CSS and JS (if any) in the template snippet.
>
>
