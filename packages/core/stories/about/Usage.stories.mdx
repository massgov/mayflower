...
### Configuration
 ...
 We provide SCSS variables and partials for organizations to configure and compile their own Mayflower stylesheets, based on individual branding needs. You can configure all the variables declared in `scss/00-base` with a !default flag, such as `_variables.scss` `_colors.scss` `_color-tokens.scss` `_breakpoints.scss` `_z-index.scss` For example to theme the Mayflower header with your own primary color:
