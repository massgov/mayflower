...
Accompanying shades and tints can be used to create depth. In general, the lighter variants are used for outline, and the lightest variants are used as background colors. Avoid using them as text colors. 

### Primary Colors

**Primary Color** is the default and most often used color on a page and throughout the site.  **Examples:** Headings, titles, icons, graphics, calls to action, buttons,
and backgrounds.
