...
Note: colors in Mayflower codebase are referenced as css properties, with prefix `--mf-` to the color name, for example: `--mf-c-primary`.
...
For examples, `--mf-c-bay-blue`, as the **Primary Color**, is the default color for links and primary action buttons. Accompanying shades and tints can be used to create depth. It's also used in headings, titles, icons, graphics, and calls to action. `--mf-c-berkshires-green`, as the **Alternate Primary Color**, usually does the same job as the primary color, but when the surrounding context is different, for example in a call-to-action block. `--mf-c-duckling-yellow`, as the **Highlight Color**, is used for time sensitive information like updates and alerts. "Revolution Gray" or `--mf-c-black`, the darkest gray on the gray scale, is primarily used as base text color.
...
<ul className="sg-colors">
  {
    primaryColors.map((color, i) => <li key={`primaryColors${i}`}><ColorSwatch {...color} variable={color.token}/></li>)
  }
</ul>
...
<ul className="sg-colors">
  {
    primaryAltColors.map((color, i) => <li key={`primaryAltColors${i}`}><ColorSwatch {...color} variable={color.token}/></li>)
  }
</ul>

### Highlight Colors
The purpose of the highlight colors are to to draw attention to key sections or information that usually is time sensitive. **Examples:** Updates, time changes, alerts. When being used as background colors, always pair with the dark text color `--mf-c-black`.

<ul className="sg-colors">
  {
    highLightColors.map((color, i) => <li key={`highLightColors${i}`}><ColorSwatch {...color} variable={color.token}/></li>)
  }
</ul>
...
<ul className="sg-colors">
  {
    grayScaleColors.map((color, i) => <li key={`grayScaleColors${i}`}><ColorSwatch {...color} variable={color.token}/></li>)
  }
</ul>
...
Some brand colors like `--mf-c-duckling-yellow` and `--mf-c-berkshires-green` are doing double duty as semantic colors while others like `--mf-c-bay-blue` and `--mf-c-independence-cranberry` are not. We wanted to keep our palette compact, but be careful not to use `--mf-c-bay-blue` as a focus color or `--mf-c-independence-cranberry` as a warning color.

<ul className="sg-colors">
  {
    utilityColors.map((color, i) => <li key={`utilityColors${i}`}><ColorSwatch {...color} variable={color.token}/></li>)
  }
</ul>
