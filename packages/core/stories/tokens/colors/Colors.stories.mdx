...
For examples, `var(--mf-c-bay-blue)`, as the **Primary Color**, is the default color for links and primary action buttons. Accompanying shades and tints can be used to create depth. It's also used in headings, titles, icons, graphics, and calls to action. `var(--mf-c-berkshires-green)`, as the **Alternate Primary Color**, usually does the same job as the primary color, but when the surrounding context is different, for example in a call-to-action block. `var(--mf-c-duckling-yellow)`, as the **Highlight Color**, is used for time sensitive information like updates and alerts. "Revolution Gray" or `var(--mf-c-black)`, the darkest gray on the gray scale, is primarily used as base text color.
...
### Highlight Colors
The purpose of the highlight colors are to to draw attention to key sections or information that usually is time sensitive. **Examples:** Updates, time changes, alerts. When being used as background colors, always pair with the dark text color `var(--mf-c-black)`.
...
Some brand colors like `var(--mf-c-duckling-yellow)` and `var(--mf-c-berkshires-green)` are doing double duty as semantic colors while others like `var(--mf-c-bay-blue)` and `var(--mf-c-independence-cranberry)` are not. We wanted to keep our palette compact, but be careful not to use `var(--mf-c-bay-blue)` as a focus color or `var(--mf-c-independence-cranberry)` as a warning color.
